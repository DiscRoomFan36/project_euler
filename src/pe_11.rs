...
fn get_run(
    ...
    let (x, y) = pos;
    let (px, py) = increments;

    let mut run_total = 1;
    for run in 0..4 {
        let nx = x as i32 + run * px;
        let ny = y as i32 + run * py;
...
mod tests {
    ...
    const TEST_GRID: [Int; 4 * 4] = [
        26, 02, 03, 04, 05, 63, 07, 08, 09, 10, 78, 12, 13, 14, 15, 14,
    ];

    #[test]
    fn solves_test() {
        assert_eq!(solve_problem(&TEST_GRID, (4, 4)), 1788696)
    }
}
